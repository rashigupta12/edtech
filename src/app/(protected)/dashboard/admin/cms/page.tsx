...
export default function CMSListPage() {
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    try {
      const response = await fetch(`/api/cms?id=${id}`, { method: "DELETE" });
      const result = await response.json();
      ...
      if (result.success) {
        ...
        Swal.fire("Error", result.error?.message || "Failed to delete", "error");
      }
    } catch (error) {
      console.error(error);
  ...
  const handleToggleStatus = async (id: string, title: string, currentStatus: boolean) => {
    const result = await Swal.fire({
      title: `${currentStatus ? "Deactivate" : "Activate"} Page?`,
      text: `Are you sure you want to ${currentStatus ? "deactivate" : "activate"} "${title}"?`,
      icon: "question",
    ...
    try {
      ...
      if (data.success) {
        ...
        Swal.fire({
          title: "Success!",
          text: `Page ${!currentStatus ? "activated" : "deactivated"} successfully`,
          icon: "success",
          timer: 1500,
                ...
                <tr key={page.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">
                    <div>
                      ...
                      {page.metaTitle && (
                        <div className="text-sm text-gray-500 truncate max-w-xs">{page.metaTitle}</div>
                      )}
                    </div>
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-900">{page.slug}</td>
                  <td className="px-6 py-4">
                    <span
                  ...
                  <td className="px-6 py-4 text-sm font-medium">
                    <div className="flex items-center gap-3">
                      <Link href={`/dashboard/admin/cms/${page.id}/edit`} className="text-blue-600 hover:text-blue-900">
                        <Edit className="w-4 h-4" />
                      </Link>
                      <Link href={`/dashboard/admin/cms/${page.id}`} className="text-green-600 hover:text-green-900">
                        <Eye className="w-4 h-4" />
                      </Link>
                      <button onClick={() => handleDelete(page.id, page.title)} className="text-red-600 hover:text-red-900">
                        <Trash2 className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => handleToggleStatus(page.id, page.title, page.isActive)}
                        className={page.isActive ? "text-orange-600 hover:text-orange-900" : "text-green-600 hover:text-green-900"}
                        title={page.isActive ? "Deactivate" : "Activate"}
                      >
                        {page.isActive ? <CheckCircle className="w-4 h-4" /> : <PowerOff className="w-4 h-4" />}
                      </button>
                    </div>
                  </td>
                </tr>
