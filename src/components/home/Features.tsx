...
export default function Features() {
  const features = [
    ...
    },
    {
      ...
      description: "Earn recognized certificates upon course completion Industry-recoginzed",
      points: [
        "Industry-Recognized",
        "Portfolio Ready",
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Experience learning reimagined with our cutting-edge platform
            features
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature, index) => (
            <div key={index} className="group relative">
              {/* Glass Morphism Card */}
              <div
                className={`relative backdrop-blur-sm bg-white/70 ${getColorClass(
                  feature.accentColor,
                  "border"
                )} border rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-500 hover:scale-105 overflow-hidden`}
              >
                ...
                <div className="relative mb-6">
                  <div
                    className={`w-14 h-14 ${getColorClass(
                      feature.accentColor,
                      "bg"
                    )} rounded-xl flex items-center justify-center transform group-hover:rotate-12 transition-transform duration-300`}
                  >
                    <feature.icon
                      className={getColorClass(feature.accentColor, "text")}
                      size={28}
                    />
                  </div>
                  <div
                    className={`absolute -top-2 -right-2 w-6 h-6 ${getColorClass(
                      feature.accentColor,
                      "bg"
                    )} rounded-full flex items-center justify-center`}
                  >
                    <span
                      className={`text-xs font-bold ${getColorClass(
                        feature.accentColor,
                        "text"
                      )}`}
                    >
                ...
                {/* Content */}
                <h3
                  className={`relative text-xl font-bold text-gray-900 mb-4 group-hover:${getColorClass(
                    feature.accentColor,
                    "text"
                  )} transition-colors`}
                >
